# Mallam Town — Mystery & Clue Ledger

Trimmed to the mysteries that should drive chapters (per the earlier structural note — ten was too many for a reader to actively track). Secondary threads live at the bottom as **texture, not tracked boxes**. For each core mystery: the true answer, what points toward it, what misdirects, and where it's planted vs. paid off. Fill in the chapter numbers as you draft — that's the whole point of this ledger, to catch a clue planted twice or a payoff with no setup.
...
- **Community blames land guards for the Camry incident** (Day 0 evening) — plants the land-guard association early, without specifying whose crew.
- **Chocho is enlisted by police to help run the manhunt for Kwame, deploying his own men.** Because of the earlier gossip, this should read, at the time, as faintly suspicious — the reader may wonder if Chocho's men are "helping" search in order to control the outcome. That suspicion is a genuine red herring: it's *resolved*, not confirmed, when Chocho later identifies the real (outside) culprits from the recording. His cooperation turns out to be sincere.

**Add to the plan:**
- A plant establishing land-guard activity in Mallam Town early (Act I, ideally in the same texture as the preacher/PA-system worldbuilding)
- Chocho needs at least one scene *before* the robbery so his later implication isn't a cold introduction
- The gossip blaming land guards for the Camry (Day 0 evening, alongside "Bad Luck *Nkoaa*") — see Master Timeline
- Chocho's men joining the Day 1 manhunt — see Master Timeline
- The recording scene where Chocho identifies the voice and names the outside group — see Master Timeline / Mystery 5
...
| Element | Detail | Chapter (fill in) |
|---|---|---|
| Plant | Establish early that Kwame always drives his own car, never lends it, so its use in a robbery reads as impossible to everyone who knows him | — |
| Plant / character beat | His refusal to hand over the car (or even take on a paying fare) even at gunpoint is *in character* — the same immovability that made him beloved as a peacemaker is what escalates this into violence | — |
| Payoff | **Now staged directly on the page** (per the decided mechanism above) rather than left as an offstage fact — this closes what was the single biggest plausibility gap in the plot | — |
...
## CORE MYSTERY 6 — The masked man / the cemetery

**Status:** Upgraded from pure texture — it now has a genuine plant/payoff structure worth tracking, since the resolution ties directly back to it.
...
## SECONDARY THREADS (character-level, not mystery boxes)
...
## How to use this as you draft

Every time you plant or pay off one of these, fill in the chapter number in the table. Before finishing any chapter, scan this ledger for anything you were supposed to plant or pay off there — that's the whole value of keeping it separate from the prose.
